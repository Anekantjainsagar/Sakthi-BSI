...
"""
    ...
    async def _run_gemini_calls_parallel(self):
        """T2.6: Run all Gemini calls in parallel using asyncio"""
        import asyncio
        
        # Create tasks for each Gemini call
        tasks = [
            asyncio.to_thread(self.ai_apt_threat_mapping),
            asyncio.to_thread(self.ai_attack_vector_correlation),
        ]
    ...
    def run_correlation_direct(self, phase1_data: Dict, phase2_data: Dict, phase3_data: Dict, dark_web_data: Dict = None):
        """
        T1.3: Direct data passing without temp files.
        Accepts phase data directly instead of loading from file paths.
        """
        # quiet logs for web
        ...
        # Load phases directly from data
        try:
            ...
            if not phase3_data or not isinstance(phase3_data, dict):
                return {"error": "Phase 3 data required"}
            
            self.phase3_data = phase3_data
            self.domain = phase3_data.get('domain', '')
        except Exception as e:
            return {"error": f"Phase data loading error: {e}"}
        ...
        # T2.6: Parallelize all Gemini calls
        if self.use_gemini:
            import asyncio
            try:
                ...
                if loop.is_running():
                    ...
                    # Run in parallel
                    loop.run_until_complete(self._run_gemini_calls_parallel())
            except Exception:
