...
# T1.4: Import global connection pool manager
from core.connection_pool import ConnectionPoolManager
# T2.4: Import cache manager
...
# T2.4: Initialize cache manager (48-hour TTL for Phase 2)
cache_manager = CacheManager(cache_dir=".cache/phase2", ttl_hours=48)
...
# ✅ T1.2: REDUCED PORT LIST (Top 100 most critical ports for faster scanning)
# Prioritized by: frequency of exploitation + business impact
COMMON_PORTS = [
    # Web Services (8 ports)
    80, 443, 8080, 8443, 8000, 8888, 8001, 8002,
    # Remote Access (6 ports)
    22, 23, 3389, 5900, 5901, 5902,
    # Email Services (7 ports)
    25, 110, 143, 465, 587, 993, 995,
    # Databases (8 ports)
    3306, 5432, 1433, 27017, 6379, 9200, 5984, 7000,
    # File Sharing (5 ports)
    21, 69, 445, 139, 135,
    # DNS/Network (4 ports)
    53, 67, 161, 162,
    # Directory Services (3 ports)
    389, 636, 3268,
    # VPN/Tunneling (4 ports)
    1194, 1723, 500, 4500,
    # Proxy/Cache (3 ports)
    3128, 8118, 9090,
    # Management/Admin (6 ports)
    111, 2049, 5985, 5986, 10250, 6443,
    # Container/Cloud (4 ports)
    2375, 2376, 6379, 9200,
    # Monitoring (4 ports)
    9100, 9200, 9300, 8086,
    # Message Queues (4 ports)
    5672, 61613, 61614, 27017,
    # Development (5 ports)
    3000, 5000, 8000, 8080, 9000,
    # Other Critical (6 ports)
    1521, 11211, 7001, 7002, 8009, 9999,
]

# Remove duplicates and sort
COMMON_PORTS = sorted(list(set(COMMON_PORTS)))[:100]  # Limit to top 100
...
class BSIInfrastructureDiscovery:
    ...
    async def __aenter__(self):
        # T1.4: Use global connection pool instead of creating new session
        self.session = ConnectionPoolManager.get_aiohttp_session()
        return self

    async def __aexit__(self, exc_type, exc_val, exc_tb):
        # Don't close the global session here - it's managed globally
        pass
    ...
    async def _crtsh_subdomains(self, target: str, data: InfrastructureData):
        """T2.4: Cached certificate transparency lookup"""
        # Check cache first
        cache_key = {'source': 'crtsh', 'target': target}
        cached = cache_manager.get('crtsh_subdomains', cache_key)
        if cached:
            logger.info(f"✅ crt.sh cache hit for {target}")
                ...
                if self.session:
                    async with self.session.get(url, timeout=30) as response:
                        if response.status == 200:
                            ...
                            # T2.4: Cache the results
                            cache_manager.set('crtsh_subdomains', cache_key, subdomains_found)
    ...
    async def _verify_active_subdomains(self, data: InfrastructureData):
        """T2.3: Increased batch size (50→200) and reduced sleep (1.0s→0.2s) for faster verification"""
        subdomains_to_check = data.subdomains[:500]
        batch_size = 200  # T2.3: Increased from 50 to 200
        for i in range(0, len(subdomains_to_check), batch_size):
            ...
            await asyncio.sleep(0.2)  # T2.3: Reduced from 1.0s to 0.2s
